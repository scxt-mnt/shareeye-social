...
const DashBoardNav = () => {
    ...
    return (
        <>
            ...
            <main className='w-full h-[60px] bg-violet-700 flex flex-ro~w items-center gap-2 sticky top-0 z-50'>
                {isFocus === false &&
                    <figure>
            ...
            {isFocus &&
                <>
                    {selector.length !== 0 ?
                        <section className='w-[18rem] h-auto bg-white absolute rounded-xl flex flex-col pb-2 pt-2 shadow-lg items-center gap-4 max-h-[25rem] overflow-auto'>
                            ...
                            {selector.map(((fields) => {
                                return (
                                    ...
                                    <Link to={`View-Profile/${fields.id}`}><section key={fields.id} className='w-auto h-auto pt-3 pb-3 pl-2 flex flex-row gap-2 text-lg font-poppins -ml-10 border-l-2 border-violet-900'>
                                        ...
                                        <p key={'name'}>{fields.name}</p>
                                        <p key={'lastName'}>{fields.lastName}</p>
                                    </section></Link>)
                            }))}
